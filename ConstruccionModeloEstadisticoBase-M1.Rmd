---
title: "ConstruccionModeloEstadisticoBase"
...
```{r}
m = tapply(mediaEdad, edad, mean)
s = tapply(mediaEdad, edad, sd)
n = tapply(mediaEdad, edad, length)
sprintf("Medias de la edad: ")
m
sprintf("Desviación estándar de la edad: ")
s
sprintf("Tamaño de la muestra de la edad: ")
n
```
...
```{r}
plot(0,ylim=c(0,3),xlim=c(0,1), yaxt="n", ylab="",xlab="Concentración",main="Edad de peces")
...
abline(v=mean(mediaEdad),lty=3,col="red")
```
```{r}
boxplot(mediaEdad ~ edad, col = 2:5, horizontal = TRUE)
abline(v = mean(mediaEdad), lty = 3, col = "red")
```
```{r}
Tu=TukeyHSD(A)
...
```{r}
pairs(medidas,labels=c("X7","X3","X4","X5","X6"),main="Matriz de dispersión",pch=20)
```
...
```{r}
R1=lm(X7~X3+X4+X5+X6, data = medidas)
S=summary(R1)
S
```
